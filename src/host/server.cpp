...
void HdcServer::GetDaemonAuthType(HSession hSession, SessionHandShake &handshake)
{
    ...
    */
    ...
    WRITE_LOG(LOG_DEBUG, "daemon encrypt-tcp negotiated %s for %s session",
...
struct PskBufferGuard {
    unsigned char *buf = nullptr;
    size_t size = 0;
    explicit PskBufferGuard(unsigned char *b, size_t s) : buf(b), size(s) {}
    ~PskBufferGuard()
    {
        if (buf != nullptr) {
    ...
    PskBufferGuard(const PskBufferGuard&) = delete;
    PskBufferGuard& operator=(const PskBufferGuard&) = delete;
};
...
bool HdcServer::ServerSessionSSLInit(HSession hSession, SessionHandShake &handshake)
{
    ...
    PskBufferGuard guard{out.get(), BUF_SIZE_DEFAULT2};
