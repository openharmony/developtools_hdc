...
class TestMediaFile:
    ...
    def test_media_file_import_merge(self):
        recv_file = get_local_path(os.path.join(self.local_recv_dir, self.local_recv_file))
        send_dir = get_local_path(self.local_recv_dir)
        ...
        try:
            ...
            send_cmd = f"file send {recv_file} {MEDIA_PHOTO_DIR}"

            send_output = get_shell_result(send_cmd)

            assert "FileTransfer finish" in send_output

            remote_sent_file = f"{MEDIA_PHOTO_DIR}/{self.local_recv_file}"
            ls_check = get_shell_result(f"shell ls -l {remote_sent_file}")
            assert self.local_recv_file in ls_check
            ...
            check_hdc_cmd(f"shell rm -rf {REMOTE_SNAPSHOT_PATH}")
            check_hdc_cmd(f"shell rm -rf {MEDIA_PHOTO_OTHER_DIR}/test.jpeg")
            check_hdc_cmd(f"shell rm -rf {MEDIA_PHOTO_DIR}/test_recv.jpeg")
    ...
    def test_media_file_import_same_dir_merge(self):
        ...
        try:
            ...
            send_cmd = f"file send {send_dir} {MEDIA_PHOTO_DIR}"

            send_output = get_shell_result(send_cmd)

            assert "FileTransfer finish" in send_output

            remote_sent_file = f"{MEDIA_PHOTO_OTHER_DIR}/{self.local_recv_file}"
            ls_check = get_shell_result(f"shell ls -l {remote_sent_file}")
            assert self.local_recv_file in ls_check
            ...
            check_hdc_cmd(f"shell rm -rf {REMOTE_SNAPSHOT_PATH}")
            check_hdc_cmd(f"shell rm -rf {MEDIA_PHOTO_OTHER_DIR}/test.jpeg")
            check_hdc_cmd(f"shell rm -rf {MEDIA_PHOTO_OTHER_DIR}/{self.local_recv_file}")
