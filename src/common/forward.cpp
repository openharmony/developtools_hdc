...
bool HdcForwardBase::SetupTcpListenAllIp(HCtxForward ctxPoint, int port, int &rc)
{
    ...
    if (rc != 0) {
        LogMsg(MSG_FAIL, "[E001107]%s is not available", g_forwardListenIp.c_str());
    ...
    if (rc != 0) {
        ...
        if (rc == -EAFNOSUPPORT) {
            ...
            if (rc != 0) {
                LogMsg(MSG_FAIL, "[E001108]%s:%d bind failed", ipv4.c_str(), port);
                uv_strerror_r(rc, buffer, BUF_SIZE_DEFAULT);
            ...
            LogMsg(MSG_FAIL, "[E001109]%s:%d bind failed", g_forwardListenIp.c_str(), port);
            uv_strerror_r(rc, buffer, BUF_SIZE_DEFAULT);
