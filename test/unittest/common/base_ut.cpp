...
HWTEST_F(BaseTest, GenerateSubserverLogFileName_Format, TestSize.Level0) {
    setenv("OHOS_HDC_SUBSERVER_LOG_FILE", "10", 1);
    std::string fileName = Base::GenerateSubserverLogFileName();
    EXPECT_FALSE(fileName.empty());
    EXPECT_TRUE(fileName.find("hdc_subserver_") != std::string::npos);
    EXPECT_TRUE(fileName.find(".log") != std::string::npos);
    unsetenv("OHOS_HDC_SUBSERVER_LOG_FILE");
}

HWTEST_F(BaseTest, ListDirectoryFiles_EmptyDir, TestSize.Level0) {
    std::string emptyDir = "/tmp/test_empty_dir_" + std::to_string(getpid());
    mkdir(emptyDir.c_str(), 0755);
    std::vector<std::string> files = Base::ListDirectoryFiles(emptyDir);
    EXPECT_TRUE(files.empty());
    rmdir(emptyDir.c_str());
}

HWTEST_F(BaseTest, ListDirectoryFiles_NonExistDir, TestSize.Level0) {
    std::string nonExistDir = "/tmp/non_exist_dir_" + std::to_string(getpid());
    std::vector<std::string> files = Base::ListDirectoryFiles(nonExistDir);
    EXPECT_TRUE(files.empty());
}

HWTEST_F(BaseTest, CleanupSubserverLogFiles_EnvNotSet, TestSize.Level0) {
    unsetenv("OHOS_HDC_SUBSERVER_LOG_FILE");
    Base::CleanupSubserverLogFiles();
}

HWTEST_F(BaseTest, CleanupSubserverLogFiles_EnvSet, TestSize.Level0) {
    setenv("OHOS_HDC_SUBSERVER_LOG_FILE", "5", 1);
    Base::CleanupSubserverLogFiles();
    unsetenv("OHOS_HDC_SUBSERVER_LOG_FILE");
}

HWTEST_F(BaseTest, GenerateSubserverLogFileName_NoEnv, TestSize.Level0) {
    unsetenv("OHOS_HDC_SUBSERVER_LOG_FILE");
    std::string fileName = Base::GenerateSubserverLogFileName();
    EXPECT_TRUE(fileName.empty());
}

HWTEST_F(BaseTest, GenerateSubserverLogFileName_ExceedLimit, TestSize.Level1) {
    setenv("OHOS_HDC_SUBSERVER_LOG_FILE", "30", 1);
    std::string fileName = Base::GenerateSubserverLogFileName();
    EXPECT_FALSE(fileName.empty());
    unsetenv("OHOS_HDC_SUBSERVER_LOG_FILE");
}

HWTEST_F(BaseTest, GetCaller_Success, TestSize.Level1) {
    Base::Caller caller = Base::GetCaller();
    EXPECT_TRUE(caller == Base::Caller::SERVER ||
                caller == Base::Caller::CLIENT ||
