...
HWTEST_F(HdcTransferTest, TestSimpleFileIO, TestSize.Level0)
{
    ...
    // case: uv_fs_write error
    uint8_t *payload = new uint8_t[bufLen];
    memset_s(payload, bufLen, 1, bufLen);
    result = mockHdcdTransfer->SimpleFileIO(&context, context.indexIO, payload, bufLen);
    ASSERT_EQ(result, -1);
}
...
HWTEST_F(HdcTransferTest, TestRecvIOPayload, TestSize.Level0)
{
    ...
    result = mockHdcdTransfer->RecvIOPayload(&context, payload, BUF_SIZE_TINY);
    ASSERT_EQ(result, false);
}
